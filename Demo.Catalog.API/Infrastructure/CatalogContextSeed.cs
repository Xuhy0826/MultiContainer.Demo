...
using Demo.Catalog.API.Infrastructure.Extension;
...
namespace Demo.Catalog.API.Infrastructure
{
    public class CatalogContextSeed
    {
        ...
        private IEnumerable<CatalogType> GetCatalogTypesFromFile(string contentRootPath, ILogger<CatalogContextSeed> logger)
        {
            ...
            try
            {
                string[] requiredHeaders = { "catalogtype", "catalogtypedescription" };
            ...
            return File.ReadAllLines(csvFileCatalogTypes)
                                        ...
                                        .SelectTry(column => CreateCatalogType(column, csvheaders))
                                        .OnCaughtException(ex => { logger.LogError(ex, "EXCEPTION ERROR: {Message}", ex.Message); return null; })
                                        .Where(x => x != null);
        }

        private CatalogType CreateCatalogType(string[] columns, string[] headers)
        {
            if (columns.Count() != headers.Count())
            {
                throw new Exception($"column count '{columns.Count()}' not the same as headers count'{headers.Count()}'");
            }
            string typeName = columns[Array.IndexOf(headers, "catalogtype")].Trim('"').Trim();
            string typeDescription = columns[Array.IndexOf(headers, "catalogtypedescription")].Trim('"').Trim();
        ...
        private IEnumerable<CatalogType> GetPreconfiguredCatalogTypes()
        {
            ...
            {
                new CatalogType() { TypeName = "Mug", TypeDescription = "a large cup, typically cylindrical and with a handle and used without a saucer."},
                new CatalogType() { TypeName = "T-Shirt", TypeDescription = "a short-sleeved casual top, generally made of cotton, having the shape of a T when spread out flat."},
                new CatalogType() { TypeName = "Sheet" ,TypeDescription = "a large rectangular piece of cotton or other fabric, used on a bed to cover the mattress and as a layer beneath blankets when these are used."},
                new CatalogType() { TypeName = "USB Memory Stick" ,TypeDescription = "USB存储棒"}
            };
        }
        ...
        private IEnumerable<CatalogItem> GetCatalogItemsFromFile(string contentRootPath, CatalogContext context, ILogger<CatalogContextSeed> logger)
        {
            ...
            var catalogTypeIdLookup = context.CatalogTypes.ToDictionary(ct => ct.TypeName, ct => ct.Id);
